...
const News = () => {
    ...
    const currentNews = hardcodedNews.slice(firstNewsInd, lastNewsInd);

    // console.log('lastNewsInd -',lastNewsInd,'firstNewsInd -',firstNewsInd)
    
    const [active, setActive] = useState(1)
    let items = [];
    for (let number = 1; number <= Math.ceil(hardcodedNews.length / newsPerPage); number++) {
      items.push(
    ...
    return (
        <>
            ...
            {hardcodedNews.length <= newsPerPage ? '' : !loading && <PaginationBasic />}
        </>
    )
}
